...
// mission 01. 사각형 크기를 구하는 함수 (가로, 세로) -> 사각형의 크기는 ___입니다.
//함수명 : getRectArea
//결과 : 사각형의 크기는 
function getRectArea(width, height) {
  const result3 = width * height; 
  console.log(`사각형의 크기는 ${result3}입니다.`);
}

getRectArea(5, 10);   // 사각형의 크기는 50입니다.
...
function isAdult(name, age) {
  const result4 = age >= 20 ? '성인' : '미성년자';
  console.log(`${name}님은 ${result4}입니다.`);
}

isAdult('hongildong', 17); 

// mission 03. 학생의 성적표를 받아서, 평균을 구하시오. (국어, 영어, 수학)
//함수명: getAverage
//결과: 당신의 평균점수는 __입니다. (a+b+c)/3

function getAverage(a, b, c) {
  const average = (a + b + c) / 3;
  console.log(`당신의 평균점수는 ${average}점입니다.`);
}

getAverage(90, 80, 70); // 당신의 평균점수는 80.00점입니다.
...
function checkLogin(userId, userPw) {
  const result5 = userId === id && userPw ==+ pw ? "로그인 성공" : "로그인 실패";
    console.log(result5);
}
